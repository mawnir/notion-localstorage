...
const wordsDetect = (text: string, config: Config = {}) => {
    ...
    const words3 = words2.filter(word => word.trim());
    ...
    words3.forEach(function (word: string) {
        const carry = [];
        let matched;
        do {
...
const wordsCount = (text: string, config = {}) => {
    const { count } = wordsDetect(text, config);
    return count;
}

const wordsSplit = (text: string, config = {}) => {
